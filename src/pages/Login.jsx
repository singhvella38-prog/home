...
export default function SocialLoginPage() {
  ...
  const handleNaverLogin = () => {
    // 1. 네이버 개발자 센터에서 발급받은 Client ID
    const CLIENT_ID = '5ho47O_84JQdizE9Kcya'; 
    
    // 2. 네이버 개발자 센터에 등록한 Callback URL (정확해야 함)
    const REDIRECT_URI = encodeURIComponent('https://home-pearl-nine.vercel.app/api/auth/callback/naver');
    ...
    <div className="min-h-screen bg-[#f5f6f7] flex flex-col items-center justify-center p-4">
      ...
      <div className="w-full max-w-[460px] bg-white border border-gray-300 rounded-lg shadow-sm p-8">
        ...
        <div className="space-y-3">
          ...
          >
            <span className="w-5 text-center">N</span> 네이버로 로그인
          </button>

          {/* 카카오 (추후 동일한 방식으로 연결 가능) */}
          <button className="w-full flex items-center justify-center gap-3 bg-[#FEE500] text-[#191919] py-3.5 rounded-md font-bold text-[16px]">
            <span className="w-5 text-center">K</span> 카카오로 로그인
          </button>

          {/* 구글 */}
          <button className="w-full flex items-center justify-center gap-3 bg-white text-gray-700 py-3.5 rounded-md font-bold text-[16px] border border-gray-300">
            <span className="w-5 text-center">G</span> Google로 로그인
          </button>
        </div>
      ...
      <p className="mt-8 text-xs text-gray-400 uppercase">
